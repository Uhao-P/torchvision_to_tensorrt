...
if __name__ == '__main__':
      ...
      old_device = 'cpu'
      # old_device = 'cuda:0'
      old_model = init_detector(config_file, checkpoint_file, device=old_device)
      ...
      mr.load_state_dict(state_dict)

      res = mr.predict('./111.png', old_model)
      old_res = inference_detector(old_model, './111.png')

      old_im = old_model.show_result('./111.png', old_res)
      new_im = old_model.show_result('./111.png', res)

      cv2.imwrite('old_res.jpg', old_im)
      cv2.imwrite('new_res.jpg', new_im)
