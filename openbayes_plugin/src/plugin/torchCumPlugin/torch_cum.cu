...
__global__ void torch_cum_warp_kernel(T *output, const T *input, size_t stride,
  ...
  for (int index =
    ...
    for (int warp_offset = 0; warp_offset < dim_size;
      ...
      T thread_data = cum_position < dim_size
      ...
      if (cum_type == 0) {
        ...
        warpScan(temp_storage.scan[int(threadIdx.x / CUDA_WARP_SIZE)])
            .InclusiveScan(thread_data, thread_data, CumScanProd<T>(),
                           aggregate_value);
      }
...
void torch_cum(T *output, const T *input, int *input_dims, int nb_dims,
  ...
  torch_cum_warp_kernel<T><<<num_blocks, CUDA_NUM_THREADS, 0, stream>>>(
      output, input, input_stride.size[cum_dim], ts_input_size.size[cum_dim],
      cum_size, cum_type);
}
