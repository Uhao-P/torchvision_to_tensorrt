...
using openbayes::cuda::TensorSize;
using openbayes::cuda::TensorStride;
...
template <typename T, typename PreReduceOp, typename ReduceOp,
          typename PostReduceOp>
__global__ void adaptive_pool_kernel(
    T *output, const T *input, TensorSize input_size, TensorStride input_stride,
    TensorSize output_size, TensorStride output_stride, int nb_dims,
  ...
  for (int cell_id =
           blockIdx.x * cell_per_block + threadIdx.x / thread_per_cell;
       cell_id < (N); cell_id += cell_per_block * gridDim.x) {
    ...
    for (int i = 0; i < nb_dims; ++i) {
      ...
      const int input_idx_start =
          START_IND(idx, output_size.size[i], input_size.size[i]);
      const int input_idx_end =
          END_IND(idx, output_size.size[i], input_size.size[i]);
    ...
    for (int cell_data_id = cell_thread_id; cell_data_id < cell_count;
         cell_data_id += thread_per_cell) {
      ...
      if (cell_data_id == cell_thread_id) {
        smemChar[threadIdx.x] =
            pre_reduce_op(input[cell_input_offset + input_idx], cell_count);
      } else {
        smemChar[threadIdx.x] = reduce_op(
            smemChar[threadIdx.x],
            pre_reduce_op(input[cell_input_offset + input_idx], cell_count));
      }
    }
...
void adaptive_pool(T *output, const T *input, int *input_dims, int *output_dims,
  ...
  for (int i = nb_dims - nb_reduce_dims; i < nb_dims; ++i) {
    size_t reduce_size =
        ceilDiv<size_t>(ts_input_size.size[i], ts_output_size.size[i]);
