...
template <typename T>
struct unary_linear_index_to_reduce_index
    : public thrust::unary_function<T, T> {
  int nCols;

  __host__ __device__ unary_linear_index_to_reduce_index(int nCols)
      : nCols(nCols) {}
...
struct unary_divide_by_scalar : public thrust::unary_function<T, T> {
  ...
  __host__ __device__ T operator()(const T &x) const { return x / a; }
};

template <typename T>
void tensorMean(T *dst,const T *src, int *src_size, bool *reduce_dims, int dims,
  ...
  if (need_permute){
      ...
      reduce2DContigous<T>(dst, src, (T)0., src_length, dst_length,
                       thrust::identity<T>{}, reduceSumOp<T>(),
                       divScalarOp<T>(mean_length), stream);
  }
...
template void tensorMean<float>(float *dst,const float *src, int *src_size,
...
template void tensorMeanVar<float>(float *mean_dst, float *vat_dst,
                                   const float *src, int *src_size,
                                   bool *reduce_dims, int dims,
                                   cudaStream_t stream, void *workspace);
