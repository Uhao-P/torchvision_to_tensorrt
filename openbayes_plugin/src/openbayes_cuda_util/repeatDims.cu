...
void repeat_dims(T *dst, const T *src, const int *input_size,
                 const int *repeatDims, int dims, cudaStream_t stream) {
  ...
  for (int i = dims - 2; i >= 0; --i) {
    ...
    output_stride.size[i] = output_stride.size[i + 1] *
                            ts_input_size.size[i + 1] * repeatDims[i + 1];
  }

  size_t dst_elems =
      output_stride.size[0] * ts_input_size.size[0] * repeatDims[0];
...
template void repeat_dims<float>(float *dst, const float *src,
                                 const int *input_dims, const int *repeatDims,
                                 int dims, cudaStream_t stream);
template void repeat_dims<int>(int *dst, const int *src, const int *input_dims,
                               const int *repeatDims, int dims,
                               cudaStream_t stream);
template void repeat_dims<half>(half *dst, const half *src,
                                const int *input_dims, const int *repeatDims,
                                int dims, cudaStream_t stream);
template void repeat_dims<char>(char *dst, const char *src,
                                const int *input_dims, const int *repeatDims,
                                int dims, cudaStream_t stream);
