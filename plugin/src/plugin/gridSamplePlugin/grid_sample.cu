...
static __forceinline__ __device__ scalar_t grid_sampler_unnormalize_set_grad(
    scalar_t coord, int size, bool align_corners, scalar_t *grad_in) {
  if (align_corners) {
    // unnormalize coord from [-1, 1] to [0, size - 1]
...
static __forceinline__ __device__ scalar_t reflect_coordinates_set_grad(
    scalar_t in, int twice_low, int twice_high, scalar_t *grad_in) {
  if (twice_low == twice_high) {
    *grad_in = static_cast<scalar_t>(0);
    return static_cast<scalar_t>(0);
  }
...
template <typename scalar_t>
static __forceinline__ __device__ scalar_t
grid_sampler_compute_source_index_set_grad(scalar_t coord, int size,
...
template <typename scalar_t>
static __forceinline__ __device__ void safe_add_2d(scalar_t *data, int h, int w,
...
__global__ void grid_sampler_2d_kernel(
    ...
    scalar_t *output, TensorSize input_size, TensorSize gride_size,
    TensorStride input_stride, TensorStride grid_stride,
...
void create_size_stride(const int *dims, int nb_dims, TensorSize &size,
                        TensorStride &stride) {
  ...
  if (nb_dims == 4) {
    grid_sampler_2d_kernel<T>
        <<<GET_BLOCKS(count), CUDA_NUM_THREADS, 0, stream>>>(
            count, input, grid, output, ts_input_size, ts_grid_size,
            input_stride, grid_stride, output_stride, interp, padding,
            align_corners);
  } else if (nb_dims == 5) {
